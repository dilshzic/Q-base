...
class SessionRepository @Inject constructor(
    ...
    suspend fun updateAttemptAndRecalculate(attempt: SessionAttempt) {
        ...
        allAttempts.forEach { att ->
            ...
            val q = questionDao.getQuestionById(att.questionId)
            val qType = q?.questionType?.trim()?.uppercase() ?: "SBA"
            if (qType == "SBA") {
                maxPossible += 4f
            } else {
                val optCount = questionDao.getOptionsForQuestion(att.questionId).first().size
                maxPossible += optCount.toFloat().coerceAtLeast(1f)
            }
        }

        val scorePercentage = if (maxPossible > 0) (totalMarks / maxPossible) * 100 else 0f
        
        sessionDao.getSessionById(attempt.sessionId)?.let { session ->
            sessionDao.updateSession(session.copy(scoreAchieved = scorePercentage.coerceAtLeast(0f)))
        }
    }
